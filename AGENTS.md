...
- `zsh/`, `git/`, `starship/`, `ghostty/`, `rime/`: package directories that mirror target paths in `$HOME` (for example `zsh/.zshrc` becomes `~/.zshrc`).
...
- `./setup.sh` — stow core packages (`zsh`, `git`, `starship`, `ghostty`, `rime`) into `$HOME`.
